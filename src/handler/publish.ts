...
export async function handler(event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> {
  const result = await tolerateAllErrors({ procedure: "handler.publish", event }, async () =>
    (await inject().PublisherService()).publishTask(getValidatedInput(event, PublishRequest)),
  );
